...
const S2N_BRANCH_ALIGN_FLAG: &str = "-Wa,-mbranches-within-32B-boundaries";

fn contains_s2n(value: &OsStr) -> bool {
    value.to_string_lossy().contains("s2n-tls-sys")
}

fn normalized_path(value: &OsStr) -> String {
    value.to_string_lossy().replace('\\', "/")
}

fn is_s2n_real_source(value: &OsStr) -> bool {
    let path = normalized_path(value);
    path.ends_with(".c")
        && (path.starts_with("lib/") || path.contains("/lib/"))
        && !path.contains("lib/tests/features/")
}
...
fn main() -> ExitCode {
    ...
    if use_clang_for_s2n {
        let strip_branch_align = args.iter().any(|arg| is_s2n_real_source(arg.as_os_str()));
        ...
        command.args(
            args.into_iter()
                .filter(|arg| !(strip_branch_align && arg == S2N_BRANCH_ALIGN_FLAG)),
        );
    } else {
