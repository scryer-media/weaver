...
func main() {
	...
	flag.StringVar(&multiVolumeBytesPerFile, "multi-volume-bytes-per-file", "40MiB", "target size for each movie in the multi-input fixtures")
	flag.StringVar(&compressibleBytesPerFile, "compressible-bytes-per-file", "270MiB", "target size for each compressible movie file (its archive is roughly 62-70% of this)")
