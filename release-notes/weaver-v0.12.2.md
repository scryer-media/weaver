# Weaver 0.12.2 release notes

Weaver 0.12.1 only reached the container image: two Windows build checks
failed, so its downloads, installers and GitHub release were never
published. 0.12.2 fixes those checks and changes nothing else, so these notes
describe everything since 0.12.0. If you already run the 0.12.1 container,
upgrading to 0.12.2 changes nothing.
