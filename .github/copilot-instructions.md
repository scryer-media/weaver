...
Call `list_scopes` first. Use the current repository root as the scope for this repo, or `scope = "workspace"` for cross-repo searches. MCP `pathPrefix` and `file` filters are repo-relative; never include parent workspace folders or `scryer-media` in those filters.
