...
fn extract_with_sevenz(
    ...
    sevenz_fast::decompress_with_password(source, dest, sevenz_fast::Password::empty()).unwrap();
...
fn extract_encrypted_7z() {
    ...
    let file = fs::File::open(&archive_path).unwrap();
    sevenz_fast::decompress_with_password(
        file,
        &out_dir,
        sevenz_fast::Password::new("TestPass123"),
    )
    .unwrap();
...
fn extract_empty_archive() {
    ...
    if archive_path.exists() {
        let file = fs::File::open(&archive_path).unwrap();
        match sevenz_fast::decompress_with_password(file, &out_dir, sevenz_fast::Password::empty())
        {
            Ok(()) => {
...
fn extract_with_callback_pattern() {
    ...
    sevenz_fast::decompress_with_extract_fn_and_password(
        ...
        |entry: &sevenz_fast::ArchiveEntry, reader: &mut dyn Read, _dest: &PathBuf| {
