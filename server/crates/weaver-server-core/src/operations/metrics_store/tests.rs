...
fn record_metrics_history_sample_round_trips_raw_points() {
    ...
    db.record_metrics_history_sample(
        ...
        &[job_info(1, JobStatus::Queued), job_info(2, JobStatus::Downloading)],
    )
    .unwrap();
    db.record_metrics_history_sample(
        110,
        &sample_snapshot(1_300, 900, 650, 180, 3, 6.0, 3.0),
        &[job_info(1, JobStatus::Downloading), job_info(2, JobStatus::Paused)],
    )
    .unwrap();

    let rows = db.list_metrics_history_chunks(MetricsHistoryTier::Raw10s).unwrap();
...
fn record_metrics_history_sample_builds_avg_and_peak_rollups() {
    ...
    db.record_metrics_history_sample(
        ...
        &[job_info(1, JobStatus::Queued), job_info(2, JobStatus::Queued)],
    )
    .unwrap();
    db.record_metrics_history_sample(
        20,
