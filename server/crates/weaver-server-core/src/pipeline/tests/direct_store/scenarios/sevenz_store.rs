...
///
/// Correctly, not cheaply. Base discovery deletes a set's cached facts when no
/// volume file is on disk, which is every direct set, so a restored 7z set has
/// no container map and no member list — and without a member list its
/// checkpoint digest cannot be reproduced, so the coverage row is refused and
/// the set refetches. That is the safe direction of the two: the restored set
/// re-parses the container from the articles as they arrive again and lands the
/// member byte for byte, never materializing a volume on the way.
#[tokio::test]
async fn a_sevenz_set_restarts_without_materializing_a_volume() {
    ...
    assert!(
        !queued.is_empty(),
        "a restored set with nothing to route from must ask for its articles"
    );
    assert!(
        pipeline
            .direct_store
