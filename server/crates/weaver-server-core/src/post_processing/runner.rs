...
fn prepare_execution(
    ...
    let mut roots = vec![
        request.context.working_directory.clone(),
        request.context.final_directory.clone(),
    ];
    roots.extend(
        request
            .approved_roots
...
fn adapter_environment_and_args(
    ...
    let context = &request.context;
    let success = matches!(context.pipeline_outcome, PipelineOutcome::Succeeded);
    match request.manifest.adapter() {
        ...
        ExtensionAdapter::Sabnzbd => {
            ...
            insert_env(env, "SAB_VERSION", env!("CARGO_PKG_VERSION"))?;
            insert_env(env, "SAB_NZO_ID", &context.job_id.to_string())?;
            insert_env(env, "SAB_FINAL_NAME", &context.name)?;
            insert_env(
                env,
                "SAB_CAT",
            ...
            Ok(vec![
                ...
                OsString::from(if success { "0" } else { "1" }),
                OsString::from(context.source_url.as_deref().unwrap_or_default()),
            ])
        }
        ExtensionAdapter::Nzbget => {
            if let Some(command) = request.diagnostic_command.as_deref() {
                insert_env(env, "NZBCP_COMMAND", command)?;
                insert_options(env, "NZBPO_", &request.options)?;
                return Ok(vec![]);
            }
            let total_status = if success { "SUCCESS" } else { "FAILURE" };
            ...
            insert_env(env, "NZBPP_STATUS", total_status)?;
            insert_env(env, "NZBPP_TOTALSTATUS", total_status)?;
            insert_env(env, "NZBPP_SCRIPTSTATUS", "NONE")?;
...
async fn execute_supervised(
    ...
    let mut effects = parse_control_effects(prepared.adapter, &lines)?;
...
impl BoundedOutput {
    fn push(&mut self, line: CapturedOutputLine) {
...
async fn capture_stream<R: AsyncRead + Unpin>(
    ...
    loop {
        ...
        while let Some(newline) = pending.iter().position(|byte| *byte == b'\n') {
            ...
            split_and_capture(&mut line, stream, &output, &sequence);
        }
    ...
    if !pending.is_empty() {
        split_and_capture(&mut pending, stream, &output, &sequence);
    }
    Ok(())
}
...
fn validate_effect_paths(
    ...
    let roots = roots
        .iter()
        .map(|root| canonicalize_effect_path(root))
