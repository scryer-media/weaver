...
impl RepairOutput {
    pub fn validate(&self) -> Result<(), StateError> {
        if self.filename.is_empty() || self.filename.len() > 1024
            || weaver_model::files::sanitize_download_filename(&self.filename) != self.filename
            || self.expected_length > i64::MAX as u64
        {
            return Err(StateError::Database("invalid reconstructed output identity".into()));
        }
        Ok(())
    }
}
...
impl Database {
    ...
    pub(crate) fn reserve_repair_outputs(&self, job: JobId, outputs: &[RepairOutput]) -> Result<(), StateError> {
        if outputs.len() > 16_384 { return Err(StateError::Database("too many reconstructed outputs".into())); }
        for output in outputs { output.validate()?; }
...
pub(crate) fn identity(output: &RepairOutput) -> super::record::ActiveFileIdentity {
    super::record::ActiveFileIdentity {
        ...
        classification: crate::pipeline::Pipeline::canonical_archive_identity_from_filename(&output.filename),
...
pub(crate) async fn restore_assembly(
    ...
    for output in outputs {
        ...
        let id = super::ids::NzbFileId { job_id: job, file_index: output.file_index };
        if assembly.file(id).is_some() || assembly.files().any(|file| {
            identities.get(&file.file_id().file_index).map_or(file.filename(), |identity| identity.current_filename.as_str()) == output.filename
        ...
        match tokio::fs::symlink_metadata(directory.join(&output.filename)).await {
            Ok(metadata) if metadata.file_type().is_file() && metadata.len() == output.expected_length => {
                file.mark_complete();
                present.push(output.file_index);
            },
            Ok(metadata) if !metadata.file_type().is_file() => return Err("reconstructed output is not a regular file".into()),
            Ok(_) => {},
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {},
            Err(error) => return Err(error.to_string()),
        }
        identities.entry(output.file_index).or_insert_with(|| identity(output));
...
mod tests {
    use super::*;
    use crate::jobs::{assembly::{FileAssembly, JobAssembly}, ids::NzbFileId, record::ActiveJob};
    use std::collections::HashMap;

    fn job() -> ActiveJob {
        ActiveJob {
            job_id: JobId(1), nzb_hash: [3; 32], nzb_path: "input.nzb".into(), nzb_zstd: vec![],
            output_dir: "output".into(), created_at: 1, category: None, metadata: vec![],
            status: "queued", download_state: "queued", post_state: "idle", run_state: "active",
            paused_resume_status: None, paused_resume_download_state: None,
            paused_resume_post_state: None, password_override: None,
        }
    }
    ...
    fn output_reservation_is_atomic_replayable_and_cascades() {
        ...
        let first = RepairOutput { file_index: 3, filename: "restored.bin".into(), expected_length: 7 };
        db.reserve_repair_outputs(JobId(1), std::slice::from_ref(&first)).unwrap();
        db.reserve_repair_outputs(JobId(1), std::slice::from_ref(&first)).unwrap();
        let mut conflicting = first.clone();
        conflicting.expected_length += 1;
        let second = RepairOutput { file_index: 4, filename: "other.bin".into(), expected_length: 0 };
        assert!(db.reserve_repair_outputs(JobId(1), &[second, conflicting]).is_err());
        assert_eq!(db.load_repair_outputs(JobId(1)).unwrap(), vec![first.clone()]);
        drop(db);
        let db = Database::open(&path).unwrap();
        assert_eq!(db.load_repair_outputs(JobId(1)).unwrap(), vec![first.clone()]);
    ...
    async fn output_restore_preserves_missing_members_without_download_progress() {
        let root = tempfile::tempdir().unwrap();
        tokio::fs::write(root.path().join("present.bin"), b"present").await.unwrap();
        let outputs = vec![
            RepairOutput { file_index: 3, filename: "present.bin".into(), expected_length: 7 },
            RepairOutput { file_index: 4, filename: "missing.bin".into(), expected_length: 9 },
        ];
        let mut assembly = JobAssembly::new(JobId(1));
        let mut identities = HashMap::new();
        assert_eq!(restore_assembly(JobId(1), &outputs, root.path(), &mut assembly, &mut identities).await.unwrap(), vec![3]);
        let present = assembly.file(NzbFileId { job_id: JobId(1), file_index: 3 }).unwrap();
        ...
        let missing = assembly.file_mut(NzbFileId { job_id: JobId(1), file_index: 4 }).unwrap();
        ...
        missing.reset();
        assert!(!missing.is_complete());
        assert!(restore_assembly(JobId(1), &outputs, root.path(), &mut assembly, &mut identities).await.is_err());
    }
    ...
    async fn output_restore_cannot_replace_download_members() {
        let root = tempfile::tempdir().unwrap();
        let mut assembly = JobAssembly::new(JobId(1));
        assembly.add_file(FileAssembly::new(NzbFileId { job_id: JobId(1), file_index: 0 }, "input.bin".into(), weaver_model::files::FileRole::Unknown, vec![10]));
        let mut identities = HashMap::new();
        for output in [
            RepairOutput { file_index: 0, filename: "different.bin".into(), expected_length: 10 },
            RepairOutput { file_index: 3, filename: "input.bin".into(), expected_length: 10 },
            RepairOutput { file_index: 3, filename: "../outside.bin".into(), expected_length: 10 },
        ] {
            assert!(restore_assembly(JobId(1), &[output], root.path(), &mut assembly, &mut identities).await.is_err());
        }
