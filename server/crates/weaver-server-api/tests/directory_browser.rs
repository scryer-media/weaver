...
async fn browse_specific_path() {
    ...
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {}) {{ currentPath parentPath entries {{ name path }} }} }}",
            gql_string(tempdir.path())
        ))
        .await;
...
async fn browse_nonexistent_path() {
    ...
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {}) {{ currentPath parentPath }} }}",
            gql_string(&missing_child)
        ))
        .await;
...
async fn browse_file_not_dir() {
    ...
    let path = gql_string(tempfile.path());

    let h = TestHarness::new().await;
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {path}) {{ currentPath }} }}"
        ))
        .await;
    assert_has_errors(&resp);
    let err_msg = format!("{:?}", resp.errors);
...
async fn browse_unreadable_path_rejected() {
    ...
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {}) {{ currentPath }} }}",
            gql_string(&unreadable)
        ))
        .await;
...
async fn browse_has_parent_path() {
    ...
    let subdir_path = gql_string(&subdir);

    let h = TestHarness::new().await;
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {subdir_path}) {{ currentPath parentPath }} }}"
        ))
        .await;
    assert_no_errors(&resp);
    let data = response_data(&resp);
...
async fn browse_entries_sorted() {
    ...
    let dir_path = gql_string(tempdir.path());

    let h = TestHarness::new().await;
    let resp = h
        .execute(&format!(
            "{{ browseDirectories(path: {dir_path}) {{ entries {{ name }} }} }}"
        ))
        .await;
    assert_no_errors(&resp);
    let data = response_data(&resp);
...
async fn create_directory_creates_folder_and_returns_listing() {
    let tempdir = tempfile::TempDir::new().expect("failed to create temp dir");
    let parent_path = gql_string(tempdir.path());
    let created_path = tempdir.path().join("alpha");

    let h = TestHarness::new().await;
    let resp = h
        .execute(&format!(
            "mutation {{ createDirectory(path: {parent_path}, name: \"alpha\") {{ currentPath parentPath entries {{ name path }} }} }}"
        ))
        .await;
...
async fn create_directory_rejects_duplicate_name() {
    ...
    let resp = h
        .execute(&format!(
            "mutation {{ createDirectory(path: {parent_path}, name: \"alpha\") {{ currentPath }} }}"
        ))
        .await;
...
async fn create_directory_rejects_invalid_name() {
    ...
    let resp = h
        .execute(&format!(
            "mutation {{ createDirectory(path: {parent_path}, name: \"nested/child\") {{ currentPath }} }}"
        ))
        .await;
