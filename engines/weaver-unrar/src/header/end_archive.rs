...
pub fn parse(raw: &RawHeader) -> RarResult<EndArchiveHeader> {
    ...
    let (end_flags, _) = read_raw_vint_lossy(body);
...
mod tests {
    ...
    fn build_end_header(end_flags: u64) -> Vec<u8> {
        ...
        build_end_header_from_type_body(&type_body)
    }

    fn build_end_header_from_type_body(type_body: &[u8]) -> Vec<u8> {
