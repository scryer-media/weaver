...
//! RAR4/RAR15 signature: 7 bytes `52 61 72 21 1A 07 00`
//! RAR14 signature: 4 bytes `52 45 7E 5E`
//!
...
/// Returns `ArchiveFormat::Rar14` for RAR 1.4 archives.
...
pub fn detect_format(header: &[u8]) -> RarResult<ArchiveFormat> {
    ...
    if header.len() >= RAR14_SIGNATURE.len() && header[..RAR14_SIGNATURE.len()] == RAR14_SIGNATURE {
        return Ok(ArchiveFormat::Rar14);
    }
...
fn scan_for_signature<R: Read + Seek>(reader: &mut R, max_scan: u64) -> RarResult<Option<u64>> {
    scan_for_signature_range(reader, 0, 0, max_scan)
}

fn scan_for_signature_range<R: Read + Seek>(
    reader: &mut R,
    archive_base: u64,
    scan_start: u64,
    scan_len: u64,
    ...
    let mut file_offset: u64 = scan_start;
    let scan_end = scan_start.saturating_add(scan_len);
    let mut carry = 0usize; // how many bytes carried over from previous chunk

    loop {
        // Don't scan beyond the requested range.
        if file_offset >= scan_end {
            return Ok(None);
        }

        let remaining = (scan_end - file_offset) as usize;
        ...
        if prefix.len() < 32 {
            let needed = 32 - prefix.len();
            prefix.extend_from_slice(&buf[carry..carry + n.min(needed)]);
        }
        ...
        while let Some(pos) = find_subsequence(&window[rar14_search_from..], &RAR14_SIGNATURE) {
            ...
            let absolute = file_offset - carry as u64 + pos as u64;
            if rar14_sfx_candidate_allowed(archive_base, absolute, scan_start, &prefix) {
                return Ok(Some(absolute));
            }
            rar14_search_from = pos + 1;
        }
...
fn rar14_sfx_candidate_allowed(
    archive_base: u64,
    absolute: u64,
    scan_start: u64,
    prefix: &[u8],
    ...
    let marker_start = archive_base.saturating_add(28);
    let marker_end = marker_start.saturating_add(RAR14_SFX_MARKER.len() as u64);
    if scan_start >= marker_start || scan_start.saturating_add(prefix.len() as u64) < marker_end {
        return true;
    }
    let rel = (marker_start - scan_start) as usize;
    prefix
        .get(rel..rel + RAR14_SFX_MARKER.len())
        .is_some_and(|marker| marker == RAR14_SFX_MARKER)
}
...
/// with `InvalidSignature`, performs a bounded scan (up to 4 MiB) to support
...
pub fn read_signature<R: Read + Seek>(reader: &mut R) -> RarResult<ArchiveFormat> {
    ...
    if let Ok(()) = direct_result {
        match detect_format(&buf) {
            ...
            Ok(ArchiveFormat::Rar14) => {
                // RAR14's RE~^ marker is part of the 7-byte main header.
                reader.seek(SeekFrom::Start(start)).map_err(RarError::Io)?;
                return Ok(ArchiveFormat::Rar14);
            }
    ...
    match scan_for_sfx_signature(reader, start, DEFAULT_SFX_MAX_SCAN)? {
        Some(offset) => {
            ...
            match fmt {
                ...
                ArchiveFormat::Rar14 => {
                    // RAR14 parsing starts at the RE~^ marker.
                    reader.seek(SeekFrom::Start(offset)).map_err(RarError::Io)?;
                }
                ArchiveFormat::Rar5 => {}
            }
...
mod tests {
    ...
    #[test]
    fn test_rar14_signature() {
        let mut data = [0u8; 8];
        data[..RAR14_SIGNATURE.len()].copy_from_slice(&RAR14_SIGNATURE);
        let result = detect_format(&data);
        assert!(matches!(result, Ok(ArchiveFormat::Rar14)));
    }
    ...
    fn test_sfx_rar14_signature() {
        ...
        let result = read_signature(&mut cursor);
        assert!(matches!(result, Ok(ArchiveFormat::Rar14)));
        assert_eq!(cursor.position(), stub_size as u64);
    }
    ...
    fn test_sfx_exceeds_max_scan() {
        // Place the signature beyond the 4 MiB scan limit.
    ...
    fn test_sfx_signature_just_under_scan_limit() {
        ...
        assert!(matches!(result, Ok(ArchiveFormat::Rar5)));
        assert_eq!(cursor.position(), (stub_size + RAR5_SIGNATURE_LEN) as u64);
    }
