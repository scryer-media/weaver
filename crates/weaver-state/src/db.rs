...
impl Database {
    ...
    fn create_schema(&self) -> Result<(), StateError> {
        ...
        match version {
            ...
            Some(2) | Some(3) => {
                // v2/v3→v7: add appended column + priority column; RSS tables are created above.
                conn.execute_batch(
            ...
            Some(4) => {
                // v4→v7: add appended column + priority column; RSS tables are created above.
                conn.execute_batch(
                    "ALTER TABLE active_extraction_chunks ADD COLUMN appended INTEGER NOT NULL DEFAULT 0;
                     ALTER TABLE servers ADD COLUMN priority INTEGER NOT NULL DEFAULT 0;",
                )
            ...
            Some(5) => {
                // v5→v7: add priority column to servers; RSS tables are created above.
                conn.execute_batch(
                    "ALTER TABLE servers ADD COLUMN priority INTEGER NOT NULL DEFAULT 0",
                )
                .map_err(|e| StateError::Database(e.to_string()))?;
