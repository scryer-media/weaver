...
func TestPar3OutputRestartE2E(t *testing.T) {
	...
	for _, name := range []string{"restored.bin", "downloaded.bin"} {
		...
		for file, body := range carrier {
			if name == "restored.bin" && (file == name || strings.Contains(file, ".vol")) {
				continue
			}
			if strings.HasSuffix(file, ".par3") {
				file = prefix + file
			}
			posted[file] = body
		}
	...
	for index, name := range unpackSortedNames(posted) {
		if !strings.HasPrefix(name, "second.") || !strings.Contains(name, ".vol") {
			continue
		}
	...
	for time.Now().Before(deadline) {
		var records int
		err := db.QueryRow("SELECT COUNT(*) FROM active_repair_outputs WHERE job_id = $1 AND filename = $2", job, "restored.bin").Scan(&records)
	...
	if !ready {
		t.Fatalf("never reached durable reconstructed-output wait: status=%s log=%s", api.status(job), firstLog)
	}
	stop()
	damaged := bytes.Clone(expected["restored.bin"])
	damaged[170000] ^= 0x80
	if err := os.WriteFile(restored, damaged, 0644); err != nil {
		t.Fatal(err)
	}
	release()
	...
	for name, want := range expected {
		...
		if err != nil || !bytes.Equal(actual, want) {
			t.Fatalf("wrong restored output %s: %v", name, err)
		}
	}
	var records int
	if err := db.QueryRow("SELECT COUNT(*) FROM active_repair_outputs WHERE job_id = $1", job).Scan(&records); err != nil || records != 0 {
