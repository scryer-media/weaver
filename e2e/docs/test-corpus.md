...
| `test-corpus/toolchains.json` | The generator-toolchain lock: five RARLAB writers, the official 7-Zip console binary, par2cmdline-turbo, the par3cmdline reference, UUDeview, the digest-pinned FFmpeg encoder image, and the Go writers, each pinned by URL and SHA-256 or by image digest. A toolchain change is a deliberate corpus revision. |
